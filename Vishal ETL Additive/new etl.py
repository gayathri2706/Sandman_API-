...
def run_etl(config, engine, connection, target_table):
    ...
    df_add = pd.read_sql("SELECT * FROM mixer ORDER BY ID DESC LIMIT 10000", engine)
    prod_data = pd.read_sql("SELECT * FROM consumption_booking_test", engine)
 
    # Convert datetime string to pandas datetime
    df_add['datetime'] = pd.to_datetime(df_add['Date_Time'], format='%Y-%m-%d %H:%M:%S')
    ...
    column_pairs = [
        ("NEW_SAND_SET_KGS", "NEW_SAND_ACT_KGS_REPORT"),
        ("BENTONITE_SET_KGS", "BENTONITE_ACT_KGS_REPORT"),
        ("RETURN_SAND_1_SET_KGS", "RETURN_SAND_ACT_KGS_REPORT"),
        ("FINE_SET_KGS", "FINE_DUST_ACT_KGS_REPORT"),
        ("COAL_SET_KGS", "COAL_DUST_ACT_KGS_REPORT"),
        ("WATER_SET_FLOW", "WATER_ACT_FLOW"),
    ]
    ...
    if last_timestamp is not None:
        ...
        df.to_excel("output_full.xlsx", index=False)  # Save full output for review
    ...
    # df.to_excel("output_sample.xlsx", index=False)  # Save sample to Excel for review
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            print(f"Next ETL run in 600 seconds. Press Ctrl+C to exit cleanly.")
